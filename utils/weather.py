...
def get_weather_data(lat, long, arrival_date_str=None, trip_duration=7):
    try:
        ...
        arrival_date = today # Fallback default

        # 1. Catch the dates passed from the UI
        if arrival_date_str:
            arrival_date = datetime.strptime(arrival_date_str, "%Y-%m-%d").date()
            if (arrival_date - today).days > 14:
                needs_historical = True

        # Calculate exact end date for the API to fetch
        end_date = arrival_date + timedelta(days=min(trip_duration - 1, 13))

        # --- ROUTE A: LIVE FORECAST (<= 14 Days) ---
        if not needs_historical:
            start_str = arrival_date.strftime("%Y-%m-%d")
            end_str = end_date.strftime("%Y-%m-%d")
            
            # Notice we are now using exact start/end dates instead of forecast_days=16
        ...
        # --- ROUTE B: 3-YEAR HISTORICAL AVERAGE (> 14 Days) ---
        else:
